...
class _EditProfileState extends State<EditProfile> {
  ...
  void initState() {
    ...
    profileController.text =
        widget.studentProfile.userDescription!.join(', ');
                ...
                children: [
                  ...
                  Flexible(
                    child: RadioListTile<SelectOptions>(
                      title: Text(
                        'A company',
                        style: TextStyle(color: widget.studentProfile.userType == "Person"? Color(0xffA3A3A3) : Colors.black, fontSize: 14), // Reduced font size
                      ),
                      visualDensity: VisualDensity(horizontal: -4), // Adjusted visual density
                      value: SelectOptions.company,
                      groupValue: _option,
                      fillColor: widget.studentProfile.userType == "Person" ? MaterialStateProperty.all(Color(0xffA3A3A3)) : MaterialStateProperty.all(Colors.black),
                      onChanged: widget.studentProfile.userType == "Person" ? null : (SelectOptions? value) {
                  ...
                  Flexible(
                    child: RadioListTile<SelectOptions>(
                      title: Text(
                        'A person',
                        style: TextStyle(color: widget.studentProfile.userType == "Company" ?  Color(0xffA3A3A3) : Colors.black, fontSize: 14), // Reduced font size
                      ),
                      visualDensity: VisualDensity(horizontal: -4), // Adjusted visual density
                      value: SelectOptions.person,
                      groupValue: _option,
                      fillColor: widget.studentProfile.userType == "Company" ? MaterialStateProperty.all(Color(0xffA3A3A3)) : MaterialStateProperty.all(Colors.black),

                      onChanged: widget.studentProfile.userType == "Company" ? null : (SelectOptions? value) {
                        if (value == null) return;
                        setState(() {
                          _option = value;
                        });
    ...
    return  GestureDetector(
      onTap:  () {
        ...
        ProfileServices().updateUserProfile(
            ...
            imgUrl: isUpdated? "" : widget.studentProfile.userProfilePicture! ,
            isUpdated: isUpdated,
          firstName: firstName,
          lastName: lastName
        ).then((value) {
